...
class coevo2(EvoAlgo):
    ...
    def run(self, maxsteps):
        ...
        print("Coevo2 seed %d Popsize %d %d batchSize %d stepsize %lf noiseStdDev %lf wdecay %d sameEnvCond %d nparams %d" % (self.seed, self.popsize, self.selsize, self.batchSize, self.stepsize, self.noiseStdDev, self.wdecay, self.sameenvcond, nparams))
        ...
        for gen in range(self.ngenerations):
            ...
            if (((gen + 1) % self.policy.saveeach) == 0):
                filename = "S%dG%d.npy" % (seed, gen + 1)
                np.save(filename, pop)
                filename = "S%dFitG%d.npy" % (seed, gen + 1)
                np.save(filename, self.fmatrix)
            if (((gen + 1) % (self.policy.saveeach * 10)) == 0):
